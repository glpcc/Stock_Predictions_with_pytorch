...
net.eval()
epochs = len(norm_test_data)//batch_size - 2
b = Bar('Generating...',max=epochs)
for epoch in range(epochs):
    test_index = 1 + epoch*batch_size
    inpt = norm_test_data[test_index]

    for batch_num in range(batch_size):
        output = net(inpt)
        inpt = output
        real_open_prices.append(unnormalize(float(norm_test_data[test_index+batch_num][0])))
...
pyplot.title('LSTM 20 Long term (5 days) Predicitons')
pyplot.grid(True)
pyplot.show()
